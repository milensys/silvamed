  ...
  {foreach $addresses as $address}
    ...
    >
      ...
      <footer class="address-footer">
        {if $interactive}
          <a
            class="edit-address btn btn-sm btn-success"
            data-link-action="edit-address"
            href="{url entity='order' params=['id_address' => $address.id, 'editAddress' => $type, 'token' => $token]}"
          >
            <i class="linearicons-pencil mr-1" aria-hidden="true"></i>{l s='Edit' d='Shop.Theme.Actions'}
          </a>
          <a
            class="delete-address btn btn-sm btn-dark"
            data-link-action="delete-address"
            href="{url entity='order' params=['id_address' => $address.id, 'deleteAddress' => true, 'token' => $token]}"
          >
            <i class="linearicons-trash2 mr-1" aria-hidden="true"></i>{l s='Delete' d='Shop.Theme.Actions'}
          </a>
        {/if}
      </footer>
    </article>
