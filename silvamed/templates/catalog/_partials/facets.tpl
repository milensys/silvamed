                ...
                {foreach from=$facet.filters key=filter_key item="filter"}
                  {if $filter.displayed}
                    <li>
                      {if $facet.type == 'price'}
                        <div class="p-1 pt-2">
                          <p id="price-slider-label"><strong>{l s='Range:' d='Shop.Theme.Global'}</strong> <span>{$facet.properties.min} - {$facet.properties.max}</span></p>
                          ...
                          <a href="{$filter.nextEncodedFacetsURL}" id="price-slider-link" class="d-none search-link js-search-link" data-currency="{$currency.sign|escape:"url"}" rel="nofollow"></a>
                        </div>
                        {break}
