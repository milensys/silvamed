    ...
    {if $post}
      <div class="text-center blog-post-wrapper">
        ...
        <div class="post-meta d-flex flex-wrap align-items-center justify-content-center">
          ...
          <div{if !$displayAuthor} class="d-none"{/if}><p>{l s='by [0][1"]%link%["1]%name%[/1][/0]'
              sprintf=[
              ...
              d='Shop.Theme.Global'}</p>
          </div>
          {if $displayViews && $post.views}
          <div>
            <em class="mb-2 d-inline-block">{l s='Views' mod='jxblog'} ({$post.views})</em>
          </div>
          {/if}
        </div>
        ...
        {if $tags}
          <div class="post-tags my-3">
            <h5 class="mr-2 mb-1">{l s='Tagged' mod='jxblog'}</h5>
            {foreach from=$tags item='tag' name="tag"}
              <a class="mb-1" href="{$link->getModuleLink('jxblog', 'tag', ['id_jxblog_tag' => $tag.id_jxblog_tag])}">{$tag.tag}</a>{if !$smarty.foreach.tag.last},{/if}
            {/foreach}
          </div>
        {/if}
      </div>
