  ...
  <section id="main">
    <div id="mywishlists">
      ...
      <header class="page-header">
        <h2 class="text-center products-section-title">{l s='My wishlists' mod='jxwishlist'}</h2>
      </header>
      ...
      {if $id_customer|intval neq 0}
        <section class="page-content mb-3 mb-lg-5">
          <form method="post" class="std box mx-auto text-center" id="form_wishlist" >
            <fieldset class="card">
              <div class="card-block">
                <h3 class="card-header">{l s='New wishlist' mod='jxwishlist'}</h3>
                <div class="card-body">
                  <div class="form-group text-left">
                    <label class="form-control-label" for="name">{l s='Name' mod='jxwishlist'}</label>
                  ...
                  <footer class="form-footer clearfix">
                    <button id="submitWishlists" class="btn btn-primary btn-md form-control-submit pull-xs-right" name="submitWishlists" type="submit">
            ...
            <li data-wishlist-id="{$wishlist.id_wishlist|intval}" id="wishlist_{$wishlist.id_wishlist|intval}" data-wishlist-name="{$wishlist.name|truncate:22:'...'|escape:'html':'UTF-8'}">
              <h3 class="jx-wishlist-title">
                ...
                <a class="delete-wishlist" href="#" onclick="javascript:event.preventDefault();return (WishlistDelete('wishlist_{$wishlist.id_wishlist|intval}', '{$wishlist.id_wishlist|intval}', '{l s='Do you really want to delete this wishlist ?' mod='jxwishlist' js=1}'));">
                  <i class="fa fa-trash-o linearicons-trash2" aria-hidden="true"></i>
                </a>
                <a class="edit-wishlist" href="#" onclick="WishlistEdit('{$wishlist.id_wishlist|intval}');">
                  <i class="linearicons-pencil" aria-hidden="true"></i>
                </a>
              </h3>
                  ...
                  <ul class="row">
                    {foreach from=$products item=product name=product}
                      {if $product.id_wishlist == $wishlist.id_wishlist}
                        <li class="clp_{$product.id_product|escape:'htmlall':'UTF-8'}_{$product.id_product_attribute|escape:'htmlall':'UTF-8'} col-12 col-sm-4 col-md-3 col-lg-2 text-center">
                          <div class="product-thumbnail">
                            ...
                            <a class="lnkdel close-product" href="javascript:;" onclick="DeleteProduct('{$wishlist.id_wishlist|intval}', '{$product.id_product|intval}', '{$product.id_product_attribute|intval}')" title="{l s='Delete' mod='jxwishlist'}">
                              <i class="icon-remove-sign linearicons-cross2"></i>
                            </a>
                          </div>
                          <h3>
                            <a class="product-title" href="{$link->getProductlink($product.id_product, $product.link_rewrite)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">
                              <span class="quantity-formated"><span class="quantity">{$product.quantity|intval}</span> x </span>{$product.name|truncate:40:'...'|escape:'html':'UTF-8'}
                            </a>
                          </h3>
                        </li>
                      {/if}
              ...
              <div class="clearfix wishlist-row-bottom">
                <a target="_blank" href="{$link->getModuleLink('jxwishlist', 'wishlist', ['token' => $wishlist.token])|escape:'htmlall':'UTF-8'}" class="btn btn-default btn-md btn-view-wishlist mr-2 my-1">
                  {l s='View wishlist' mod='jxwishlist'}
                </a>
                {if $products}
                  <button type="button" id="add-new-layout" class="btn btn-social btn-facebook mr-3 my-1">
                    <i class="fa fa-facebook" aria-hidden="true"></i>
                    <span>{l s='Share' mod='jxwishlist'}</span>
                  </button>
                {/if}
      ...
      <footer class="page-footer">
        <a href="{$urls.pages.my_account}" class="account-link btn btn-default btn-md mr-3 mt-2">
          <span>{l s='Back to your account' d='Shop.Theme.Customeraccount'}</span>
        </a>
        <a href="{$urls.pages.index}" class="account-link btn btn-default btn-md mt-2">
