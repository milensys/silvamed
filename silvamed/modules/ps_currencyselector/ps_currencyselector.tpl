...
<div class="currency-selector js-dropdown dropdown">
  <div class="dropdown-menu dropdown-menu-left" aria-labelledby="currency-selector-label">
    {foreach from=$currencies item=currency}
      <a title="{$currency.name}" rel="nofollow" href="{$currency.url}" class="dropdown-item{if $currency.current} active{/if}">{$currency.iso_code}</a>
    {/foreach}
  </div>
  <select class="custom-select js-link" aria-labelledby="currency-selector-label">
    {foreach from=$currencies item=currency}
